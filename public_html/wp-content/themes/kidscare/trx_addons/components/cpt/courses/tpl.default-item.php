...
<div data-post-id="<?php the_ID(); ?>" class="sc_courses_item trx_addons_hover trx_addons_hover_style_links">
	...
	?><div class="sc_courses_item_info">
        ...
        <div class="sc_courses_item_wrap"><div class="sc_courses_item_meta">
            <a href="<?php the_permalink(); ?>">
                        <span class="sc_courses_item_meta_item sc_courses_item_meta_date"><?php
                            $dt = $meta['date'];
                            echo sprintf($dt < date_i18n('Y-m-d') ? esc_html__('Started on %s', 'kidscare') : esc_html__('Starting %s', 'kidscare'), '<span class="sc_courses_item_date">' . date_i18n(get_option('date_format'), strtotime($dt)) . '</span>');
                            ?></span>
                <span class="sc_courses_item_meta_item sc_courses_item_meta_duration"><?php echo esc_html($meta['duration']); ?></span>
            </a>
        </div><div class="sc_courses_item_price">
			<a href="<?php
			...
			if (!empty($args['more_text']) || (!empty($meta['product']) && (int) $meta['product'] > 0)) {
				?><div class="trx_addons_hover_links"><?php
					...
					if (!empty($meta['product']) && (int) $meta['product'] > 0) {
						?><a href="<?php the_permalink($meta['product']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'trx_addons_hover_link2', 'sc_courses', $args)); ?>"><?php esc_html_e('Buy Now', 'kidscare'); ?></a><?php
					}
				?></div><?php
			}
		?></div><?php
