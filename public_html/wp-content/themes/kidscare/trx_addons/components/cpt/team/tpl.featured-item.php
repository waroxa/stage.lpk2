...
<div data-post-id="<?php the_ID(); ?>" <?php post_class( 'sc_team_item' . (empty($post_link) ? ' no_links' : '') ); ?>>
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
								...
								apply_filters('trx_addons_filter_args_featured', array(
											...
											'hover' => 'zoomin',
											'thumb_size' => kidscare_get_thumb_size('height'),
											), 'team-featured')
								);
	?>
    <div class="sc_team_item_info">
        ...
        if (!empty($link) && !empty($args['more_text'])) {
            ?><div class="sc_team_item_button"><a href="<?php echo esc_url($link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button', 'sc_team', $args)); ?>"><?php echo esc_html($args['more_text']); ?></a></div><?php
        }
        ?></div>
</div>
<?php
