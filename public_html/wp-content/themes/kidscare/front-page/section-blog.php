<div class="front_page_section front_page_section_blog<?php
	$kidscare_scheme = kidscare_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $kidscare_scheme ) && ! kidscare_is_inherit( $kidscare_scheme ) ) {
		echo ' scheme_' . esc_attr( $kidscare_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( kidscare_get_theme_option( 'front_page_blog_paddings' ) );
?>"
		<?php
		$kidscare_css      = '';
		$kidscare_bg_image = kidscare_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$kidscare_anchor_icon = kidscare_get_theme_option( 'front_page_blog_anchor_icon' );
	$kidscare_anchor_text = kidscare_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( kidscare_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$kidscare_css      = '';
			$kidscare_bg_mask  = kidscare_get_theme_option( 'front_page_blog_bg_mask' );
			$kidscare_bg_color_type = kidscare_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $kidscare_bg_color_type ) {
				$kidscare_bg_color = kidscare_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$kidscare_caption = kidscare_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $kidscare_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $kidscare_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $kidscare_caption, 'kidscare_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$kidscare_description = kidscare_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $kidscare_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $kidscare_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $kidscare_description ), 'kidscare_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
				...
				if ( ! kidscare_exists_trx_addons() ) {
					...
					kidscare_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
				}
			}
			?>
			</div>
