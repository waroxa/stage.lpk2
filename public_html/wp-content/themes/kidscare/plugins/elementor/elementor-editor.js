...
jQuery( document ).ready(
	function() {
		...
		kidscare_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="kidscare_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.kidscare_post_editor' );
					if ( link.length > 0 ) {
						kidscare_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.kidscare_post_editor', function(e) {
					kidscare_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('kidscare_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function kidscare_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
