...
use Kestrel\Store_Credit\Scoped\CommerceGuys\Addressing\AddressFormat\AddressField;
use Kestrel\Store_Credit\Scoped\CommerceGuys\Addressing\AddressFormat\AdministrativeAreaType;
use Kestrel\Store_Credit\Scoped\CommerceGuys\Addressing\AddressFormat\DependentLocalityType;
use Kestrel\Store_Credit\Scoped\CommerceGuys\Addressing\AddressFormat\PostalCodeType;
...
foreach ($foundCountries as $countryCode) {
    ...
    $definition = \json_decode($definition, \true);
...
function generate_subdivisions($countryCode, array $parents, $subdivisionPaths, $languages)
{
    ...
    $subdivisions = [];
    $subdivisions[$group] = ['country_code' => $countryCode];
    if (\count($parents) > 1) {
    ...
    foreach ($subdivisionPaths as $subdivisionPath) {
        ...
        if ($translationLanguage) {
            ...
            $translation = \json_decode($translation, \true);
        ...
        if (isset($definition['sub_keys'])) {
            $subdivisions[$group]['subdivisions'][$code]['has_children'] = \true;
...
function generate_subdivision_depths($countries)
{
    ...
    foreach ($countries as $countryCode) {
        $patterns = [__DIR__ . '/subdivision/' . $countryCode . '.json', __DIR__ . '/subdivision/' . $countryCode . '-*.json', __DIR__ . '/subdivision/' . $countryCode . '--*.json'];
...
function apply_subdivision_customizations($subdivisions, $customizations)
{
    ...
    $customizations += ['_remove' => [], '_replace' => [], '_add' => [], '_add_after' => []];
    ...
    foreach ($customizations['_add_after'] as $addId => $nextId) {
        ...
        $new = [$addId => $customizations[$addId]];
...
function convert_format($countryCode, $format)
{
    ...
    $reverseCountries = ['KH', 'CN', 'HU', 'JP', 'KO', 'MG', 'TW', 'VN'];
