...
class WC_Store_Credit_Privacy extends WC_Abstract_Privacy {
	...
	public function __construct() {
		...
		$this->add_exporter( 'woocommerce-store-credit-coupon-data', __( 'Store credit for WooCommerce coupon data', 'woocommerce-store-credit' ), [ $this, 'coupon_data_exporter' ] );
		$this->add_eraser( 'woocommerce-store-credit-coupon-data', __( 'Store credit for WooCommerce coupon data', 'woocommerce-store-credit' ), [ $this, 'coupon_data_eraser' ] );
	}
	...
	public function get_privacy_message() {
		/* translators: Placeholder: %s - Marketplace privacy URL */
		$privacy_message = __(
	...
	protected function get_coupon_personal_data( $coupon, $email ) {
		$data = [
			[
				'name'  => __( 'Coupon code', 'woocommerce-store-credit' ),
				'value' => $coupon->get_code(),
			],
			[
				'name'  => __( 'Coupon amount', 'woocommerce-store-credit' ),
				'value' => $coupon->get_amount(),
			],
			[
				'name'  => __( 'Allowed emails', 'woocommerce-store-credit' ),
				'value' => $email,
			],
		];
	...
	protected function remove_coupon_personal_data( $coupon, $email ) {
		...
		$allowed_emails = array_diff( $allowed_emails, [ $email ] );
