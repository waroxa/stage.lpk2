...
class WC_Store_Credit_Cart {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_coupon_sort', [ $this, 'get_coupon_sort_callback' ] );
		add_filter( 'woocommerce_coupon_get_discount_amount', [ $this, 'coupon_get_discount_amount' ], 10, 5 );
		add_filter( 'woocommerce_coupon_custom_discounts_array', [ $this, 'coupon_get_discounts_array' ], 10, 2 );

		/*
		 * AvaTax priority: 999.
		 * Update totals after AvaTax load the taxes.
		 */
		add_action( 'woocommerce_after_calculate_totals', [ $this, 'after_calculate_totals' ], 1000 );
		add_action( 'woocommerce_checkout_create_order', [ $this, 'create_order' ] );
		add_action( 'woocommerce_store_api_checkout_order_processed', [ $this, 'create_order' ] );
	 ...
	 * @return callable
	 */
	public function get_coupon_sort_callback() {

		return [ $this, 'coupon_sort_callback' ];
	}

	/**
	 * Sorts coupons.
	 *
	 * - Store credit coupons are applied last.
	 * - Store credit coupons with lower amounts are applied first among store credit coupons.
	 *
	 * @since 5.0.0
	 * @internal
	 *
	 * @param WC_Coupon|mixed $a first coupon
	 * @param WC_Coupon|mixed $b Second coupon
	 * @return int
	 */
	public function coupon_sort_callback( $a, $b ) : int {

		$a_is_sc = wc_is_store_credit_coupon( $a );
		$b_is_sc = wc_is_store_credit_coupon( $b );

		if ( $a_is_sc && $b_is_sc ) {

			// both are store credit, sort by amount ascending
		...
		if ( $a_is_sc ) {
			return 1; // a is store credit, b is not: a goes last
		}

		if ( $b_is_sc ) {
			return -1; // b is store credit, a is not: b goes last
		}

		// fallback for non-store-credit coupons or invalid coupons
		if ( ( ! $a instanceof WC_Coupon || ! $b instanceof WC_Coupon ) || ( $a->get_amount() === $b->get_amount() ) ) {
			return 0;
		}
	...
	protected function fix_discounts_for_deposits( $coupon ) {
		$deposit_discounts     = WC()->session->get( 'deposits_present_discounts', [] );
		$deposit_tax_discounts = WC()->session->get( 'deposits_discount_tax', [] );
