...
function wc_store_credit_currency_has_decimals( $currency ) {
	return ( ! in_array( $currency, [ 'HUF', 'JPY', 'TWD' ], true ) );
}
 ...
 */
function wc_store_credit_get_template( $template_name, $args = [] ) {
 ...
 * @return string
 */
function wc_store_credit_get_template_html( $template_name, $args = [] ) {
	ob_start();
...
function wc_store_credit_log( $message, $level = 'notice', $handle = 'wc_store_credit', $logger = null ) {
	...
	if ( method_exists( $logger, $level ) ) {
		call_user_func( [ $logger, $level ], $message, [ 'source' => $handle ] );
	} else {
	...
	foreach ( $combine_amounts as $amounts ) {
		foreach ( $amounts as $key => $amount ) {
			...
			if ( ! isset( $merged_amounts[ $key ] ) ) {
				$merged_amounts[ $key ] = ( $recursive ? [] : 0 );
			}

			if ( $recursive ) {
				$merged_amounts[ $key ] = wc_store_credit_combine_amounts( [ $merged_amounts[ $key ], $amount ] );
			} else {
...
function wc_store_credit_discount_type_keys( $type ) {
	$types = [
		'base'     => [ 'cart', 'shipping' ],
		'tax'      => [ 'cart_tax', 'shipping_tax' ],
		'base_tax' => [ 'cart', 'cart_tax', 'shipping', 'shipping_tax' ],
		'taxes'    => [ 'cart_taxes', 'shipping_taxes' ],
		'cart'     => [ 'cart', 'cart_tax', 'cart_taxes' ],
		'shipping' => [ 'shipping', 'shipping_tax', 'shipping_taxes' ],
	];

	$keys = ( isset( $types[ $type ] ) ? $types[ $type ] : [] );
