 ...
 */
function wc_store_credit_get_product_category_terms( $args = [] ) {
	$defaults = [
...
function wc_store_credit_get_product_categories_choices( $sub_cats = false ) {
	$choices    = [];
	$categories = wc_store_credit_get_product_category_terms( [ 'parent' => 0 ] );

	foreach ( $categories as $category ) {
		$choices[ $category->term_id ] = wc_store_credit_get_product_category_choice_label( $category );

		if ( $sub_cats ) {
			$children = wc_store_credit_get_product_category_terms( [ 'parent' => $category->term_id ] );
