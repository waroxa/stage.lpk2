 ...
 */
function wc_store_credit_get_customer( $customer ) : ?WC_Customer {
	try {
		if ( is_numeric( $customer ) ) {
			$customer = new WC_Customer( (int) $customer );
		}
		return is_a( $customer, 'WC_Customer' ) && 0 !== $customer->get_id() ? $customer : null;
 ...
 * @since 3.0.0
 *
 * @param mixed $customer customer object, email or ID
 * @return string|null
 */
function wc_store_credit_get_customer_email( $customer ) : ?string {

	$customer_email = is_email( $customer ) ? $customer : false;

	if ( ! $customer_email ) {
		$customer = wc_store_credit_get_customer( $customer );
...
function wc_store_credit_get_customer_coupons( $the_customer, $status = 'active' ) {
	...
	if ( ! $customer_email || ! in_array( $status, [ 'all', 'active', 'expired', 'exhausted' ], true ) ) {
		return false;
	}
