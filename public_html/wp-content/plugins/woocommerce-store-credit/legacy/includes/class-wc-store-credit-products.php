...
class WC_Store_Credit_Products {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_order_item_needs_processing', [ $this, 'order_item_needs_processing' ], 10, 2 );
		add_action( 'woocommerce_store_credit_add_to_cart', 'woocommerce_simple_add_to_cart' );

		add_action( 'woocommerce_order_status_pending', [ $this, 'copy_coupon_data' ], 10, 2 );
		add_action( 'woocommerce_order_status_on-hold', [ $this, 'copy_coupon_data' ], 10, 2 );
		add_action( 'woocommerce_order_status_processing', [ $this, 'copy_coupon_data' ], 5, 2 );
		add_action( 'woocommerce_order_status_completed', [ $this, 'copy_coupon_data' ], 5, 2 );

		// Before sending the email with priority 10.
		add_action( 'woocommerce_order_status_processing', [ $this, 'process_purchased_credit' ], 7, 2 );
		add_action( 'woocommerce_order_status_completed', [ $this, 'process_purchased_credit' ], 7, 2 );
	}
