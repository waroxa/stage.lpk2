...
class WC_Store_Credit_Order {
	...
	public function __construct() {
		add_filter( 'woocommerce_order_get_total', [ $this, 'get_total' ], 10, 2 );
		add_filter( 'woocommerce_order_get_total_discount', [ $this, 'get_total_discount' ], 10, 2 );

		add_action( 'woocommerce_order_status_changed', [ $this, 'order_status_changed' ], 10, 4 );
		add_action( 'woocommerce_before_save_order_items', [ $this, 'before_save_items' ] );
		add_action( 'woocommerce_saved_order_items', [ $this, 'after_save_items' ] );
		add_filter( 'woocommerce_order_recalculate_coupons_coupon_object', [ $this, 'recalculate_coupon' ], 10, 4 );

		add_filter( 'woocommerce_coupon_get_discount_amount', [ $this, 'get_coupon_discount' ], 10, 5 );
		add_filter( 'woocommerce_coupon_custom_discounts_array', [ $this, 'coupon_get_discounts_array' ], 10, 2 );
		add_action( 'woocommerce_order_before_calculate_taxes', [ $this, 'before_calculate_taxes' ], 10, 2 );
		add_action( 'woocommerce_order_after_calculate_totals', [ $this, 'after_calculate_totals' ], 10, 2 );
		add_action( 'woocommerce_order_item_shipping_after_calculate_taxes', [ $this, 'update_shipping_item_taxes' ] );
	}
	...
	public function order_status_changed( $order_id, $from, $to, $order ) {
		...
		if ( in_array( $to, $restore_statuses, true ) ) {
			...
			$shipping_discount = wc_get_store_credit_discounts_for_order( $order, 'total', [ 'shipping' ] );
		...
		if ( in_array( $to, [ 'processing', 'completed' ], true ) ) {
			if ( 0 >= $order->get_total() && ! $order->get_payment_method() && 0 < wc_get_store_credit_used_for_order( $order_id ) ) {
				$order->set_payment_method( __( 'Store credit', 'woocommerce-store-credit' ) );
				$order->save();
			}
		}
	}
	...
	public function get_order_actions_for( $process ) {
		...
		$actions = ( isset( $actions_for[ $process ] ) ? $actions_for[ $process ] : [] );
	...
	public function after_save_items( $order_id ) {
		...
		if ( in_array( $action, $this->get_order_actions_for( 'recalculate_coupons' ), true ) ) {
			$order           = wc_get_order( $order_id );
			$order_discounts = $order ? $this->get_order_discounts( $order ) : null;
	...
	public function update_shipping_item_taxes( $order_item ) {
		...
		if ( isset( $shipping_discount_items[ $order_item_id ] ) ) {
			...
			try {
				$order_item->set_taxes( [ 'total' => $shipping_discount_item->taxes ] );
			} catch ( Exception $e ) {
				return;
			}
		}
