 ...
 */
function wc_store_credit_get_coupon_orders( $the_coupon, $args = [] ) {
	global $wpdb;
 ...
 */
function wc_store_credit_generate_coupon_code( $args = [] ) {
	$args = wp_parse_args(
		$args,
		[
			'length' => 16,
 ...
 */
function wc_store_credit_create_coupon( $amount, $args = [], $code_args = [] ) {
	...
	$props  = array_diff_key( $args, array_flip( [ 'inc_tax', 'apply_to_shipping', 'metas', 'code' ] ) );
...
/**
 * Creates a store credit coupon for the specified customer(s).
 *
 * @since 3.0.0
 *
 * @param int|int[]|string|string[]|WC_Customer|WC_Customer[] $customer customer object, email or ID (can be an array of customers)
 * @param float|int|numeric-string $amount the amount the coupon is worth
 * @param array<string, mixed> $args optional arguments
 * @param array<string, mixed> $code_args optional arguments used for generating the code
 * @return WC_Coupon|false The created coupon. False on failure.
 */
function wc_store_credit_create_coupon_for_customer( $customer, $amount, array $args = [], array $code_args = [] ) {

	if ( is_array( $customer ) ) {
		$customer_email = array_filter( array_map( 'wc_store_credit_get_customer_email', $customer ) );
	} else {
		$customer_email = wc_store_credit_get_customer_email( $customer );
	}
...
/**
 * Creates and sends a store credit coupon to the specified customer(s).
 *
 * @since 3.0.0
 *
 * @param int|int[]|string|string[]|WC_Customer|WC_Customer[] $customer customer object, email or ID (can be an array of customers)
 ...
 */
function wc_store_credit_send_credit_to_customer( $customer, $amount, array $args = [], array $code_args = [] ) {

	$coupon = wc_store_credit_create_coupon_for_customer( $customer, $amount, $args, $code_args );

	if ( $coupon ) {
		foreach ( (array) $customer as $single_customer ) {
			/**
			 ...
			 *
			 * @param WC_Coupon $coupon the coupon object
			 * @param int|string|WC_Customer $customer customer object, email or ID
			 * @param array<string, mixed> $args optional arguments
			 * @param array<string, mixed> $code_args optional arguments used for generating the code
			 */
			do_action( 'wc_store_credit_send_credit_to_customer', $coupon, $single_customer, $args, $code_args );
		}
	}
...
function wc_store_credit_get_redeem_url( $the_coupon ) {
	...
	return add_query_arg( [ 'redeem_store_credit' => rawurlencode( $coupon->get_code() ) ], $redeem_url );
}
