...
if ( ! class_exists( 'WC_Store_Credit_Email_Send_Credit', false ) ) {
	...
	class WC_Store_Credit_Email_Send_Credit extends WC_Email {
		...
		public function __construct() {
			...
			$this->title          = __( 'Send store credit', 'woocommerce-store-credit' );
			$this->description    = __( 'Email used for sending a store credit coupon to a customer.', 'woocommerce-store-credit' );
			...
			// Triggers.
			add_action( 'wc_store_credit_send_credit_to_customer_notification', [ $this, 'trigger' ], 10, 3 );
		...
		public function get_default_subject() {
			return __( 'You have received a coupon', 'woocommerce-store-credit' );
		}
		...
		public function get_default_heading() {
			return __( 'You have been given {coupon_amount} credit', 'email heading', 'woocommerce-store-credit' );
		}
		...
		public function get_default_button_text() {
			...
			/* translators: Placeholder: Button text to redeem store credit */
			return __( 'Redeem now', 'woocommerce-store-credit' );
		}
		...
		/**
		 * Initializes a mock coupon for preview purposes.
		 *
		 * @since 5.0.0
		 *
		 * @return WC_Coupon
		 */
		protected function get_preview_coupon() : WC_Coupon {

			$coupon = new WC_Coupon();

			$coupon->set_code( 'STORE-CREDIT-123' );
			$coupon->set_amount( 10.00 );
		 ...
		 */
		public function trigger( $coupon, $the_customer, $args = [] ) {
			$this->setup_locale();
		...
		public function get_content_args( $type = 'html' ) {
			return [
				...
				'email'              => $this,
				'coupon'             => ! $this->coupon && is_admin() ? $this->get_preview_coupon() : $this->coupon,
		...
		public function init_form_fields() {
			...
			$additional_fields = [
				'button_text' => [
					...
					'type'        => 'text',
					'desc_tip'    => true,
					'description' => __( 'Text for the email CTA button.', 'woocommerce-store-credit' ),
