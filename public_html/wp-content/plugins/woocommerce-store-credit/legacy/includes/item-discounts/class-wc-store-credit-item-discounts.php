...
class WC_Store_Credit_Item_Discounts {
	 ...
	 */
	public function __construct( $item_discounts = [] ) {
	...
	public function get_by_group( $group ) {
		return ( isset( $this->item_discounts[ $group ] ) ? $this->item_discounts[ $group ] : [] );
	}
	...
	public function get_discounts( $group = '' ) {
		if ( $group ) {
			return ( isset( $this->discounts[ $group ] ) ? $this->discounts[ $group ] : [] );
		}
	...
	public function get_discount( $group, $key ) {
		if ( ! $group || ! in_array( $key, [ 'base', 'tax', 'taxes' ], true ) ) {
			return [];
		}
	...
	protected function add_item_discount( $item_discount ) {
		if ( ! $item_discount instanceof WC_Store_Credit_Item_Discount ) {
			/* translators: Placeholder: %s - PHP class name */
			throw new InvalidArgumentException( sprintf( esc_html__( 'The argument must be an instance of the class %s.', 'woocommerce-store-credit' ), 'WC_Store_Credit_Item_Discount' ) );
		}
		...
		if ( ! isset( $this->item_discounts[ $item_group ] ) ) {
			$this->item_discounts[ $item_group ] = [];
		}
