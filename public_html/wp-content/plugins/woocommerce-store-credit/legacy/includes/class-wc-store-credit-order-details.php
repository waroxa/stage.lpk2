...
class WC_Store_Credit_Order_Details {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_order_item_display_meta_key', [ $this, 'display_item_meta_key' ], 10, 2 );
		add_filter( 'woocommerce_order_item_display_meta_value', [ $this, 'display_item_meta_value' ], 10, 3 );
		add_filter( 'woocommerce_hidden_order_itemmeta', [ $this, 'hide_order_item_metas' ] );
		add_filter( 'woocommerce_order_item_get_formatted_meta_data', [ $this, 'get_formatted_item_meta_data' ], 10, 2 );

		// Order totals.
		add_action( 'woocommerce_admin_order_totals_after_shipping', [ $this, 'admin_order_store_credit' ] );
		add_filter( 'woocommerce_get_order_item_totals', [ $this, 'add_order_store_credit_row' ], 10, 3 );
		add_filter( 'woocommerce_order_get_discount_total', [ $this, 'get_discount_total' ], 10, 2 );
		add_filter( 'woocommerce_order_get_discount_tax', [ $this, 'get_discount_tax' ], 10, 2 );
		add_filter( 'woocommerce_order_get_shipping_total', [ $this, 'get_shipping_total' ], 10, 2 );
		add_filter( 'woocommerce_order_get_shipping_tax', [ $this, 'get_shipping_tax' ], 10, 2 );
		add_filter( 'woocommerce_order_shipping_method', [ $this, 'shipping_method' ], 10, 2 );
	}
	...
	public function admin_order_store_credit( $order_id ) {
		...
		$total = wc_price( $credit, [ 'currency' => $order->get_currency() ] );
		...
		?>
		<tr>
			<td class="label"><?php esc_html_e( 'Store credit used:', 'woocommerce-store-credit' ); ?></td>
			<td width="1%"></td>
	...
	public function add_order_store_credit_row( $total_rows, $order, $tax_display ) {
		...
		if ( $credit ) {
			...
			if ( wc_store_credit_apply_before_tax( $order ) ) {
				...
				$offset_keys = [ 'shipping', 'discount', 'cart_subtotal' ];
			...
			$total_rows = array_merge(
				...
				[
					'store_credit' => [
						'label' => __( 'Store Credit used:', 'woocommerce-store-credit' ),
						'value' => "-{$credit}",
					],
				],
	...
	public function get_discount_tax( $discount_tax, $order ) {
		if ( wc_store_credit_apply_before_tax( $order ) ) {
			 ...
			 * The store credit discount is displayed in a different row.
			 */
			$credit = array_sum( wc_get_store_credit_discounts_for_order( $order, 'total', [ 'cart_tax' ] ) );
	...
	public function get_shipping_total( $shipping_total, $order ) {
		...
		$discounts = wc_get_store_credit_discounts_for_order( $order, 'total', [ 'shipping' ] );
	...
	public function get_shipping_tax( $shipping_tax, $order ) {
		...
		$discounts = wc_get_store_credit_discounts_for_order( $order, 'total', [ 'shipping_tax' ] );
