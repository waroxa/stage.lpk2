...
class WC_Store_Credit_Product_Addons {
	...
	public static function init() {
		// Template hooks.
		add_action( 'woocommerce_before_add_to_cart_button', [ __CLASS__, 'product_content' ] );
		add_action( 'wc_store_credit_single_product_content', [ __CLASS__, 'preset_amounts_content' ] );
		add_action( 'wc_store_credit_single_product_content', [ __CLASS__, 'custom_amount_content' ], 20 );
		add_action( 'wc_store_credit_single_product_content', [ __CLASS__, 'different_receiver_content' ], 30 );
		add_action( 'wp', [ __CLASS__, 'check_validation_errors' ] );

		add_filter( 'woocommerce_add_to_cart_validation', [ __CLASS__, 'validate_add_cart_item' ], 20, 2 );
		...
		$data = [
			'fields' => [
				'store_credit_custom_amount' => [
					...
					'label'             => sprintf( __( 'Credit amount (%s)', 'woocommerce-store-credit' ), get_woocommerce_currency_symbol() ),
					'type'              => 'number',
	...
	public static function get_receiver_fields() {
		$fields = [
			'store_credit_receiver_email' => [
				'type'        => 'email',
				'label'       => __( 'Send to', 'woocommerce-store-credit' ),
				'placeholder' => __( 'The receiver email', 'woocommerce-store-credit' ),
				'required'    => true,
			],
			'store_credit_receiver_note'  => [
				'type'        => 'textarea',
				'label'       => __( 'Message', 'woocommerce-store-credit' ),
				'placeholder' => __( 'Add a message', 'woocommerce-store-credit' ),
			],
		];
	...
	public static function add_cart_item_data( $cart_item, $product_id ) {
		...
		if ( ! empty( $_POST['send-to-different-customer'] ) && $product->allow_different_receiver() ) {
			...
			$values = array_filter( $values ); // nosemgrep
