...
abstract class WC_Store_Credit_Discounts {
	 ...
	 * @var array
	 */
	protected $shipping_discount_items = [];
	...
	public function calculate_item_discounts( $coupon, $discounting_amounts ) {
		...
		// Total amount available for discounting.
		$total_discounting_amount = array_sum( $discounting_amounts );

		// The total desired discount is the lesser of the coupon credit and the total discounting amount.
		$desired_total_discount = min( $total_discounting_amount, $coupon_credit );

		// Divide the discount between all items with higher precision.
		$discounts = wc_store_credit_get_proportional_discounts( $discounting_amounts, $desired_total_discount, 6 );

		// Round discounts to the required number of decimal places (e.g., 2 for currency).
		$rounded_discounts = array_map( function( $discount ) {
			return round( $discount, wc_get_price_decimals() );
		}, $discounts );

		// Calculate the total discount after rounding.
		$total_discount = array_sum( $rounded_discounts );

		// Calculate the difference between desired total discount and the total of rounded discounts.
		$difference = $desired_total_discount - $total_discount;

		if ( abs( $difference ) > 0 ) {
			// Find the item with the largest discounting amount.
			$max_item_key = array_keys( $discounting_amounts, max( $discounting_amounts ), true );
			$max_item_key = reset( $max_item_key );

			// Adjust the discount of this item.
			$adjusted_discount = $rounded_discounts[ $max_item_key ] + $difference;

			// Ensure the adjusted discount is not negative and does not exceed the item's amount.
			$adjusted_discount = min( $adjusted_discount, $discounting_amounts[ $max_item_key ] );
			$adjusted_discount = max( $adjusted_discount, 0 );

			$rounded_discounts[ $max_item_key ] = $adjusted_discount;
		}

		// Proceed with the adjusted discounts.
		foreach ( $rounded_discounts as $item_key => $discount ) {
			...
			if ( $inc_tax && ! $coupon_inc_tax ) {
				$rounded_discounts[ $item_key ] = $discount + $item_discount->get_discount_tax();
		...
		return ( $coupon_discount ? $rounded_discounts : [] );
	}
	...
	public function update_shipping_discount_items( $order ) {
		...
		foreach ( $shipping_discounts as $instance_id => $shipping_discount ) {
			...
			try {
				...
				$shipping_discount_item->set_taxes( [ 'total' => array_map( 'wc_store_credit_get_negative', $discount_taxes ) ] );
	...
	public function update_credit_discounts( $order ) {
		...
		foreach ( $this->coupon_discounts()->all() as $coupon_code => $coupon_discount ) {
			...
			foreach ( $discounts as $key => $discount ) {
				...
				} elseif ( in_array( $key, [ 'cart', 'cart_tax' ], true ) && isset( $coupons[ $coupon_code ] ) ) {
	...
	protected function create_shipping_discount_item( $shipping_item, $order ) {
		...
		$item->set_props(
			[
				/* translators: Placeholder: %s - Shipping method label */
				'method_title' => sprintf( __( 'Discount: %s', 'woocommerce-store-credit' ), $shipping_item->label ),
	 ...
	 */
	protected function register_coupon_discount( $coupon, $item_discounts = [] ) {
		try {
