...
class WC_Store_Credit_Admin {
	...
	public function __construct() {
		...
		add_action( 'admin_init', [ $this, 'add_notices' ], 30 );
	...
	public function init() {
		...
		add_action( 'current_screen', [ $this, 'setup_screen' ], 20 );
		add_action( 'check_ajax_referer', [ $this, 'setup_screen' ], 20 );
	}
	...
	public function setup_screen() {
		 ...
		 */
		remove_action( 'current_screen', [ $this, 'setup_screen' ], 20 );
		remove_action( 'check_ajax_referer', [ $this, 'setup_screen' ], 20 );
	}
	...
	public function enqueue_scripts() {
		...
		if ( wc_store_credit_is_settings_page() ) {
			wp_enqueue_script( 'wc-store-credit-settings', WC_STORE_CREDIT_URL . "assets/js/admin/settings{$suffix}.js", [ 'jquery' ], WC_STORE_CREDIT_VERSION, true );
		}

		if ( in_array( $screen_id, [ 'edit-shop_coupon', $send_credit_screen_id ], true ) ) {
			wp_enqueue_script( 'wc-store-credit-admin-customer-search', WC_STORE_CREDIT_URL . "assets/js/admin/customer-search{$suffix}.js", [ 'wc-enhanced-select' ], WC_STORE_CREDIT_VERSION, true );
		}

		if ( 'shop_coupon' === $screen_id ) {
			wp_enqueue_style( 'wc-store-credit-admin', WC_STORE_CREDIT_URL . 'assets/css/admin/admin.min.css', [], WC_STORE_CREDIT_VERSION );
			wp_enqueue_script( 'wc-store-credit-admin-meta-boxes-coupon', WC_STORE_CREDIT_URL . "assets/js/admin/meta-boxes-coupon{$suffix}.js", [ 'wc-admin-coupon-meta-boxes' ], WC_STORE_CREDIT_VERSION, true );
		}

		if ( 'product' === $screen_id ) {
			wp_enqueue_style( 'wc-store-credit-admin', WC_STORE_CREDIT_URL . 'assets/css/admin/admin.min.css', [], WC_STORE_CREDIT_VERSION );
			wp_enqueue_script( 'wc-store-credit-admin-meta-boxes-product', WC_STORE_CREDIT_URL . "assets/js/admin/meta-boxes-product{$suffix}.js", [ 'wc-admin-product-meta-boxes' ], WC_STORE_CREDIT_VERSION, true );
			wp_localize_script(
		...
		if ( $send_credit_screen_id === $screen_id ) {
			wp_enqueue_script( 'wc-store-credit-admin-send-credit', WC_STORE_CREDIT_URL . "assets/js/admin/send-credit{$suffix}.js", [ 'wc-store-credit-admin-customer-search', 'jquery-ui-datepicker' ], WC_STORE_CREDIT_VERSION, true );
		}
	}
