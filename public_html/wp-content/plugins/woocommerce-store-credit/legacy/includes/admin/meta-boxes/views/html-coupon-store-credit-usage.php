			...
			<thead>
				<tr>
					...
					foreach ( $columns as $key => $label ) :
						printf( '<th class="%1$s">%2$s</th>', esc_attr( $key ), esc_html( $label ) ); // phcs:ignore
					endforeach;
				...
				foreach ( $orders as $related_order ) :
					...
					<tr>
						<?php foreach ( $columns as $key => $label ) : ?>
							<?php $value = WC_Store_Credit_Meta_Box_Coupon_Usage::get_column_value( $key, $coupon, $related_order ); ?>

							<?php printf( '<td class="%1$s" data-colname="%2$s">%3$s</td>', esc_attr( $key ), esc_attr( $label ), $value ); // phpcs:ignore ?>
						<?php endforeach; ?>
					</tr>
					<?php
