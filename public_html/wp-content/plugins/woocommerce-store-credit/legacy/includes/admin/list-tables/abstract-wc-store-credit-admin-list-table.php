...
class WC_Store_Credit_Admin_List_Table extends WC_Admin_List_Table {
	...
	protected function get_render_filter_callback( $filter ) {
		...
		if ( is_callable( [ $this, "render_{$filter['id']}_filter" ] ) ) {
			$callback = [ $this, "render_{$filter['id']}_filter" ];
		} elseif ( ! empty( $filter['type'] ) && is_callable( [ $this, "render_{$filter['type']}_filter" ] ) ) {
			$callback = [ $this, "render_{$filter['type']}_filter" ];
		}
	...
	protected function get_query_filter_callback( $filter ) {
		...
		if ( is_callable( [ $this, "query_{$filter['id']}_filter" ] ) ) {
			$callback = [ $this, "query_{$filter['id']}_filter" ];
		} elseif ( ! empty( $filter['type'] ) && is_callable( [ $this, "query_{$filter['type']}_filter" ] ) ) {
			$callback = [ $this, "query_{$filter['type']}_filter" ];
		}
	...
	protected function render_select_filter( $filter ) {
		...
		$field_attributes = array_merge(
			...
			array_intersect_key( $filter, array_flip( [ 'id', 'class', 'style' ] ) ),
			[ 'name' => $filter['id'] ]
		);
		?>
