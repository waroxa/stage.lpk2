...
class WC_Store_Credit_Admin_Notices {
	 ...
	 * @var array
	 */
	private static $dismiss_notices = [];
	...
	public static function init() {
		if ( ! class_exists( 'WC_Admin_Notices' ) ) {
			include_once dirname( WC_PLUGIN_FILE ) . '/legacy/includes/admin/class-wc-admin-notices.php'; // nosemgrep
		}

		add_action( 'wp_loaded', [ __CLASS__, 'load_dismiss_notices' ], 20 );
		add_action( 'admin_print_styles', [ __CLASS__, 'add_notices' ], 20 );
	}
	...
	public static function add_dismiss_notice( $name, $notice_html = '' ) {
		if ( empty( $notice_html ) ) {
			self::$dismiss_notices = array_unique( array_merge( self::get_dismiss_notices(), [ $name ] ) );
		}
	...
	public static function remove_notice( $name ) {
		if ( in_array( $name, self::get_notices(), true ) ) {
			...
			self::$dismiss_notices = array_diff( self::get_dismiss_notices(), [ $name ] );
	...
	protected static function get_notice_content_from_file( $notice ) {
		$file = str_replace( [ 'wc_store_credit_', '_' ], [ '', '-' ], $notice );
