...
class WC_Store_Credit_Coupon_Discounts {
	 ...
	 */
	public function __construct( $coupon_discounts = [] ) {
	...
	public function has_with( $option ) {
		if ( ! in_array( $option, [ 'inc_tax', 'apply_to_shipping' ], true ) ) {
			return false;
		}
		...
		foreach ( $this->all() as $coupon_discount ) {
			if ( call_user_func( [ $coupon_discount, $option ] ) ) {
				$has_with = true;
				break;
			}
		}
	...
	protected function add_coupon_discount( $coupon_discount ) {
		if ( ! $coupon_discount instanceof WC_Store_Credit_Coupon_Discount ) {
			...
			throw new InvalidArgumentException( sprintf( esc_html__( 'The argument must be an instance of the class %s.', 'woocommerce-store-credit' ), 'WC_Store_Credit_Coupon_Discount' ) );
		}
