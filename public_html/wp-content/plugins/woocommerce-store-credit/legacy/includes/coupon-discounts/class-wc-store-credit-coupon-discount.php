...
class WC_Store_Credit_Coupon_Discount {
	 ...
	 */
	public function __construct( $the_coupon, $item_discounts = [] ) {
		$coupon = wc_store_credit_get_coupon( $the_coupon );

		if ( false === $coupon ) {
			throw new Exception( esc_html__( 'Invalid coupon.', 'woocommerce-store-credit' ) );
		}
		...
		if ( ! $item_discounts instanceof WC_Store_Credit_Item_Discounts ) {
			throw new Exception( esc_html__( 'Invalid item discounts.', 'woocommerce-store-credit' ) );
		}
