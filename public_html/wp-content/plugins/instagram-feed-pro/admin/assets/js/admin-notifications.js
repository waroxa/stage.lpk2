...
let CFFAdminNotifications = window.CFFAdminNotifications || (function (document, window, $) {
	...
	let app = {
		...
		dismiss: function (event) {
			...
			// Update counter.
			let count = parseInt(el.$adminBarCounter.text(), 10);
			if (count > 1) {
				--count;
			...
			// Remove notification.
			let $nextMessage = el.$nextMessage.length < 1 ? el.$prevMessage : el.$nextMessage,
