...
class SBI_Support_Tool
{
	...
	public static function delete_temporary_user($user_id)
	{
		...
		if (!current_user_can('delete_users')) {
			return array(
				'success' => false,
				'message' => __('You don\'t have enough permission to delete users', 'instagram-feed'),
			);
		}
		if (!wp_delete_user($user_id)) {
			return array(
				'success' => false,
				'message' => __('Cannot delete this user', 'instagram-feed'),
			);
		}

		return array(
			'success' => true,
			'message' => __('User Deleted', 'instagram-feed'),
		);
	}
	...
	public static function create_temporary_user()
	{
		if (!current_user_can('create_users')) {
			return array(
				'success' => false,
				'message' => __('You don\'t have enough permission to create users', 'instagram-feed'),
			);
		}
		$domain = str_replace(
			array(
		...
		if (is_wp_error($temp_user_id)) {
			$result = array(
				'success' => false,
				'message' => __('Cannot create user', 'instagram-feed'),
			);
		} else {
			$creation_time = time();
			...
			$result = array(
				'success' => true,
				'message' => __('Temporary user created successfully', 'instagram-feed'),
