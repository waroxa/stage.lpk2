<div class="sbi-fb-legacy-ctn sbi-fb-fs" v-if="legacyFeedsList != null && legacyFeedsList.length > 0">
	<div class="sbi-fb-lgc-top-new sbi-fb-fs" v-if="allFeedsScreen.onboarding.active">
		<div class="sbi-fb-lgc-gr">{{genericText.new}}</div>
		{{allFeedsScreen.onboarding.getStarted}}
	</div>
	<div class="sbi-fb-lgc-ctn sbi-fb-fs"
		 v-bind:class="{ 'sb-onboarding-highlight' : viewsActive.onboardingStep === 2 && allFeedsScreen.onboarding.type === 'multiple' }">
		<div class="sbi-fb-lgc-inf-ctn sbi-fb-fs">
			...
			<div class="sbi-fb-onbrd-tltp-parent" @click.prevent.default="openTooltipBig(this)">
				<div class="sbi-fb-onbrd-infotxt sb-caption sb-lighter">{{allFeedsScreen.legacyFeeds.toolTip}}
					<div v-html="svgIcons['information']"></div>
				</div>
				<div class="sbi-fb-onbrd-tltp-elem" :data-active="viewsActive.enabledToolTip == this">
					...
					<div class="sbi-fb-onbrd-tltp-txt sb-small-p sb-lighter"
						 v-for="legFeedTltp in allFeedsScreen.legacyFeeds.toolTipExpanded" v-html="legFeedTltp">
					...
					<div class="sb-pointer">
						<svg width="21" height="11" viewBox="0 0 21 11" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path
								d="M8.59642 0.811655C9.39546 -0.135356 10.8545 -0.135357 11.6536 0.811654L20.25 11H0L8.59642 0.811655Z"
								fill="white"/>
						</svg>
					</div>
				</div>
			</div>
			<a class="sbi-fb-lgc-btn-stg sbi-btn-grey sbi-fb-hd-btn sb-button-standard"
			   :href="builderUrl+'&feed_id=legacy'">
				<div v-html="svgIcons['cog']"></div>
				{{genericText.settings}}
			</a>
		</div>
		<div class="sbi-legacy-table-wrap"
			 v-bind:class="{ 'sb-onboarding-highlight' : viewsActive.onboardingStep === 3 && allFeedsScreen.onboarding.type === 'multiple' }">

			<table class="sbi-fd-legacy-feed-ctn"
				   v-if="viewsActive.legacyFeedsShown || feedsList === null || feedsList.length === 0">
						...
						<div class="sb-flex-center">
							...
							<div class="sbi-fd-lst-shortcode-cp sbi-fd-lst-btn sbi-fb-tltp-parent">
								...
								<div v-html="svgIcons['copy']"
									 @click.prevent.default="copyToClipBoard(legacyFeed.id)"></div>
							</div>
						</div>
					</td>
					<td class="sb-caption sb-lighter">
						<div class="sb-instances-cell"
							 v-if="typeof legacyFeed.instance_count !== 'undefined' && legacyFeed.instance_count !== false">
							<span>{{genericText.usedIn}} <span class="sbi-fb-view-instances sbi-fb-tltp-parent"
															   :data-active="legacyFeed.instance_count < 1 ? 'false' : 'true'"
															   @click.prevent.default="viewFeedInstances(legacyFeed)">{{legacyFeed.instance_count + ' ' + (legacyFeed.instance_count !== 1 ? genericText.places : genericText.place)}} <div
										class="sbi-fb-tltp-elem"><span>{{genericText.clickViewInstances.replace(/ /g,"&nbsp;")}}</span></div></span></span>
						</div>
					</td>
					<td class="sbi-fd-lst-actions sbi-fd-lst-dimmed sbi-fb-onbrd-tltp-parent" data-tltp-pos="right"
						@click.prevent.default="openTooltipBig()">
						<div class="sbi-fb-onbrd-tltp-elem sbi-fb-onbrd-tltp-elem-2" data-active="false">
							...
							<div class="sbi-fb-onbrd-tltp-txt sb-small-p sb-lighter"
								 v-for="legFeedTltp in allFeedsScreen.legacyFeeds.toolTipExpandedAction"
								 v-html="legFeedTltp"></div>
							<div class="sb-pointer">
		...
		<div v-if="feedsList !== null && feedsList.length > 0" class="sbi-fd-legacy-feed-toggle sbi-fb-fs"
			 :data-active="viewsActive.legacyFeedsShown" @click.prevent.default="activateView('legacyFeedsShown')">
			<span>{{viewsActive.legacyFeedsShown ? allFeedsScreen.legacyFeeds.hide : allFeedsScreen.legacyFeeds.show}}</span>
			<svg v-if="! viewsActive.legacyFeedsShown" width="11" height="7" viewBox="0 0 11 7" fill="none"
				 xmlns="http://www.w3.org/2000/svg">
				<path
					d="M1.675 0.158447L5.5 3.97511L9.325 0.158447L10.5 1.33345L5.5 6.33345L0.5 1.33345L1.675 0.158447Z"
					fill="#004D77"/>
			</svg>
			<svg v-else width="11" height="7" viewBox="0 0 11 7" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path d="M9.325 6.84167L5.5 3.02501L1.675 6.84168L0.5 5.66668L5.5 0.666676L10.5 5.66668L9.325 6.84167Z"
					  fill="#0068A0"/>
			</svg>
