		...
		<div class="sbi-fb-source-popup sbi-fb-popup-inside sbi-install-plugin-modal">
			...
			<div class="sbi-install-plugin-body sbi-fb-fs">
				...
				<div class="sbi-install-plugin-content">
					...
					<button v-if="plugins.installed && plugins.activated" class="sbi-install-plugin-btn sbi-btn-orange"
							:disabled="plugins.installed">{{genericText.installedAndActivated}}
					</button>
					<button v-else-if="plugins.installed && !plugins.activated"
							@click.prevent.default="installOrActivatePlugin(plugins, plugins.plugin, 'sbi_activate_addon')"
							class="sbi-install-plugin-btn sbi-btn-orange">
						<span v-html="svgIcons.spinner" v-if="installerStatus !== null"></span>
						{{genericText.activate}}
					</button>
					<button v-else
							@click.prevent.default="installOrActivatePlugin(plugins, plugins.download_plugin, 'sbi_install_addon')"
							class="sbi-install-plugin-btn sbi-btn-orange">
						<span v-html="svgIcons.spinner" v-if="installerStatus !== null"></span>
						{{genericText.install}}
					</button>
