...
class SB_Instagram_Connected_Account
{
	...
	/**
	 * SB_Instagram_Connected_Account constructor.
	 *
	 * @param string|array $search_term_or_account Search term or account data.
		...
		if ($search_type === 'business') {
			...
			if (isset($connected_accounts[$search_term])) {
				...
				foreach ($connected_accounts as $connected_account) {
					...
					if (strpos($connected_account['access_token'], '.') === false && (strtolower($connected_account['username']) === $search_term_lower || $connected_account['access_token'] === $search_term_lower)) {
						return $connected_account;
					}
				}
			}
	...
	public static function delete_local_avatar($username)
	{
		...
		if (!empty($avatars_info[$username]['file_name'])) {
	...
	/**
	 * Whether the local avatar file exists
	 *
	 * @param string $username Username.
	 *
	 * @return bool
	...
	/**
	 * Whether we should attempt to create a local avatar
	 *
	 * @param string $username Username.
	 *
	 * @return bool
	 ...
	 *
	 * @param array $connected_accounts Connected accounts.
	 * @return array
	 *
	 * @since 5.14.4
	 */
	...
	/**
	 * Get recently searched hashtags count
	 *
	 * @return int
	 * @since 6.0.4
	 */
	public function recently_searched_hashtags_count()
	{
		...
		return $recently_searched_data && isset($recently_searched_data) && !isset($recently_searched_data['data']) && is_array($recently_searched_data) ? count($recently_searched_data) : 0;
	}
}
