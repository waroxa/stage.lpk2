...
class SBI_Upgrader_Pro
{
	...
	protected const NAME = 'instagram';

	protected const SLUG = 'instagram-feed-pro/instagram-feed.php';

	protected const REDIRECT = 'sbi-settings';

	protected const INSTALL_INSTRUCTIONS = 'https://smashballoon.com/doc/setting-up-the-instagram-feed-pro-wordpress-plugin/?instagram';
		...
		if (!empty($file)) {
			...
			if ($plugin_basename) {
				...
				if (!is_wp_error($activated)) {
					...
					activate_plugin(plugin_basename(SBI_PLUGIN_BASENAME), '', false, true);
	...
	public static function check_license_upgraded_cron()
	{
		...
		if (
			...
			SBI_Upgrader_Pro::check_license_upgraded($sbi_license_data, $sbi_license_key);
		}
	}
	 ...
	 *
	 * @param array  $current_license_data The current license data.
	...
	public static function check_license_upgraded($current_license_data, $license)
	{
		...
		if (!is_wp_error($response)) {
			...
			if (
				isset($current_license_data['item_name'], $current_license_data['item_id'], $license_data['item_name'], $license_data['item_id'])
