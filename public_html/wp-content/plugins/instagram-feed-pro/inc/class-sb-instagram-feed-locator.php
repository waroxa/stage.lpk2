...
{
	/**
	 * The column names of the feed locator table
	 *
	 * @var array
	 */
	...
	/**
	 * The feed details that are being located
	 *
	 * @var array
	 */
	...
	/**
	 * The time that feeds are considered old and can be deleted
	 *
	 * @var int
	 */
	...
	/**
	 * The entries that match the feed being located
	 *
	 * @var array
	 */
	...
	/**
	 * This class is responsible for locating Instagram feed entries.
	 *
	 * @param array $feed_details The details of the feed to locate.
	 */
	 ...
	 *
	 * @param array $args The arguments to query the locator table.
	...
	public static function instagram_feed_locator_query($args)
	{
		...
		$offset = max(0, $page * SBI_Db::get_results_per_page());
	...
	/**
	 * Sanitizes the given group by column name.
	 *
	 * @param string $group_by The column name to group by.
	 * @return string The sanitized "GROUP BY" clause or an empty string if the column name is not allowed.
	 */
	...
	public static function legacy_instagram_feed_locator_query($args)
	{
		...
		$offset = max(0, $page * SBI_Db::get_results_per_page());
		$limit = SBI_Db::get_results_per_page();
	...
	/**
	 * Updates legacy feed data to the new builder format.
	 *
	 ...
	 *
	 * @return bool True if old feeds should be cleared.
	 *
	 * @since 5.11
	 */
	 ...
	 *
	 * @return bool True if feeds should be located.
	 *
	 * @since 5.11
	 */
	public static function should_do_locating()
	{
		...
		return apply_filters('sbi_should_do_locating', $should_do_locating);
	}

	/**
	 * Simliar to the should_do_locating method but will
	 * query to see if there is a feed with an unknown location that
	...
	public static function should_do_ajax_locating($feed_id, $post_id)
	{
		...
		return apply_filters('sbi_should_do_ajax_locating', $should_do_locating);
	}
	 ...
	 *
	 * @return int The number of unique feeds.
	 *
	 * @since 5.11
	 */
	 ...
	 *
	 * @return array The summary of the located feeds.
	 *
	 * @since 5.11
	 */
	...
	/**
	 * Processes a feed being located based on whether the record
	 * exists as well as whether an unknown location needs to be
	 * updated.
	 *
	 * @since 5.11
	 */
	public function add_or_update_entry()
	{
		...
		if (empty($this->matching_entries)) {
			...
			foreach ($this->matching_entries as $index => $matching_entry) {
				...
				$atts_diff = array_diff($matching_atts, $details_atts);
				if (empty($atts_diff)) {
	 ...
	 * @param string $location The location of the feed.
	 *
	 * @since 5.11
	 */
