...
class SBI_Integration
{
	 ...
	 * CTA HTML
	 *
	 * @param array  $feeds_list - list of feeds.
	 * @param string $type - dropdown or button.
	 * @return void
	 */
				...
				<div class="sbi-feed-block-cta-selector">
					...
					<?php if ($type == 'dropdown') : ?>
						<select class="sbi-feed-block-cta-feedselector">
							<option>
								<?php
								/* translators: Feed plugin name */
								echo sprintf(esc_html__('Select %s Feed', 'instagram-feed'), esc_html(ucfirst($info['plugin']))) ?>
							</option>
			...
			<?php else : ?>
				<a href="<?php echo esc_url(admin_url('admin.php?page=sbi-feed-builder')) ?>" target="_blank"
				   rel="noopener noreferrer" class="sbi-feed-block-cta-btn">
					<?php
					/* translators: Feed plugin name */
					echo sprintf(esc_html__('Create %s Feed', 'instagram-feed'), esc_html(ucfirst($info['plugin']))) ?>
				</a>
			<?php endif; ?>
	...
	public static function get_widget_info()
	{
		return [
			...
			/* translators: %s is the placeholder for the call-to-action description */
			...
			/* translators: Link to the Social Wall plugin */
