...
class WC_PAO_Tracker {
	...
	private static function calculate_product_data() {
		...
		foreach ( $product_addon_types as $product_addon_type ) {
			if ( ! isset( $data[ 'has_addon_' . $product_addon_type ] ) ) {
				$length                 = strlen( $product_addon_type );
				$product_addon_has_type = $wpdb->get_var(
					$wpdb->prepare(
						"
						SELECT 1
						FROM {$wpdb->postmeta}
						...
						LIMIT 1
					",
						'%s:4:"type";s:' . $length . ':"' . $product_addon_type . '";%'
					)
				);

				$data[ 'has_addon_' . $product_addon_type ] = $product_addon_has_type ? true : false;
		...
		if ( ! isset( $data['product_addons_with_hidden_options'] ) ) {
			$product_with_hidden_options_exists      = false;
			$global_addon_with_hidden_options_exists = false;

			// Is there at least 1 product with a hidden option?
			$product_with_hidden_options_exists = $wpdb->get_var(
				"
			    SELECT 1
			    FROM `{$wpdb->posts}` AS posts
			    ...
			    WHERE posts.post_type = 'product'
			        ...
			        AND postmeta.meta_value LIKE '%\"visibility\";i:0;%'
			    LIMIT 1
			"
			);

			if ( ! $product_with_hidden_options_exists ) {
				// Is there at least 1 global add-on with a hidden option?
				$global_addon_with_hidden_options_exists = $wpdb->get_var(
					"
				    SELECT 1
				    FROM {$wpdb->posts} AS posts
				    INNER JOIN {$wpdb->postmeta} AS postmeta
			...
			if ( $product_with_hidden_options_exists || $global_addon_with_hidden_options_exists ) {
				$data['product_addons_with_hidden_options'] = true;
			} else {
				$data['product_addons_with_hidden_options'] = false;
			}
	...
	private static function calculate_order_data() {
		...
		if ( ! isset( $data['in_multiple_currencies'] ) ) {
			...
			if ( WC_PAO_Core_Compatibility::is_hpos_enabled() ) {
				// Disable PHPCS sniffs as it is not possible to use prepare for database names.
	...
	private static function get_reusable_data( $key = '' ) {
		...
		if ( 'products_with_addons_count' === $key ) {
			...
			foreach ( $global_addons_array as $addon_group ) {
				$addon_group                                 = maybe_unserialize( $addon_group['addons'] );
				self::$reusable_data['global_addons_count'] += is_array( $addon_group ) ? count( $addon_group ) : 0;
			}
			...
			self::$reusable_data['global_addons_ids'] = self::$reusable_data['global_addons_count']
				? array_map( 'absint', $global_addons_array )
				: array();
