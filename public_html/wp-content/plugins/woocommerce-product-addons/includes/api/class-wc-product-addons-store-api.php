...
class WC_Product_Addons_Store_API {
	...
	public static function init() {
		...
		add_filter( 'woocommerce_hydration_request_after_callbacks', array( __CLASS__, 'filter_cart_item_data' ), 20, 3 );
	...
	*/
	...
	public static function filter_cart_item_data( $response, $server, $request ) {
		...
		foreach ( $data['items'] as &$item_data ) {
			...
			self::filter_cart_item_permalink( $item_data, $cart_item );
		}
	...
	/**
	 * Filter cart item permalink.
	 *
	 * @param array $item_data Item data.
	 * @param array $cart_item Cart item data.
	 */
	private static function filter_cart_item_permalink( &$item_data, $cart_item ) {
		$item_data['permalink'] = WC_Product_Addons_Helper::get_permalink( $item_data['permalink'] ?? null, $cart_item, $cart_item['key'] );
	}
}
