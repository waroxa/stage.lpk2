				...
				?>
					<div class="wc-pao-col2 wc-pao-addon-default-option-setting">
						<label for="wc-pao-addon-default-option-<?php echo esc_attr( $loop ); ?>">
							<?php esc_html_e( 'Default selection', 'woocommerce-product-addons' ); ?>
						</label>
						<select id="wc-pao-addon-default-option-<?php echo esc_attr( $loop ); ?>" name="product_addon_default[<?php echo esc_attr( $loop ); ?>]" class="wc-pao-addon-default-option">
							<option <?php echo '' === $default ? 'selected' : ''; ?> value="none"><?php esc_html_e( 'None', 'woocommerce-product-addons' ); ?></option>
							<?php
				...
				<div class="wc-pao-row wc-pao-addon-default-option-setting">
					<label for="wc-pao-addon-default-option-<?php echo esc_attr( $loop ); ?>">
						...
						if ( 'input_multiplier' === $addon_type ) {
							esc_html_e( 'Pre-filled quantity', 'woocommerce-product-addons' );
						} elseif ( 'custom_price' === $addon_type ) {
							esc_html_e( 'Pre-filled price', 'woocommerce-product-addons' );
						}
						?>
					</label>
					<?php
					...
					if ( 'checkbox' === $addon_type ) {
						...
						<div class="wc-pao-addon-content-option-default-header">
							<p class="wc-pao-addon-content-option-default-header-text">
								<?php esc_html_e( 'Default', 'woocommerce-product-addons' ); ?>
							</p>
							<?php echo wc_help_tip( __( 'Choose which options are pre-selected by default.', 'woocommerce-product-addons' ) ); ?>
						</div>
						<?php
					} else {
						?>
				...
				<div class="wc-pao-addon-restrictions-settings <?php echo esc_attr( $display_restrictions_settings ); ?>">
					<div class="wc-pao-addon-min-max <?php echo esc_attr( $display_min_max ); ?>">
						...
						if ( 'custom_price' === $addon_type ) {
							...
							<input type="text" name="product_addon_max[<?php echo esc_attr( $loop ); ?>]" value="<?php echo esc_attr( wc_format_localized_price( $max ) ); ?>" placeholder="<?php esc_attr_e( 'unlimited', 'woocommerce-product-addons' ); ?>" min="0" class="wc_input_price" />
							<?php
						} else {
							?>
