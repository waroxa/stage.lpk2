...
class WC_Product_Addons_Admin {
			...
			if ( is_array( $import_addons ) && ! empty( $import_addons ) ) {
				...
				wp_send_json_success(
					array(
						...
						'message' => sprintf( __( '%d add-ons were successfully imported.', 'woocommerce-product-addons' ), count( $import_addons ) ),
					)
				);
	...
	public function script_styles() {
		...
		$params = array(
			...
			'i18n'     => array(
				...
				'no_visible_options'        => __( 'There must be at least 1 visible option for required add-ons.', 'woocommerce-product-addons' ),
				...
				'options_show_option'       => __( 'Click to make this option visible', 'woocommerce-product-addons' ),
				'options_hide_option'       => __( 'Click to hide this option', 'woocommerce-product-addons' ),
			),
		);
		...
		if ( ! WC_PAO()->is_current_screen( array( 'product' ) ) ) {
			...
			$params = array(
				...
				'i18n_configure'                           => __( 'Configure', 'woocommerce-product-addons' ),
				'i18n_edit'                                => __( 'Edit', 'woocommerce-product-addons' ),
				'i18n_form_error'                          => __( 'Failed to initialize form. If this issue persists, please reload the page and try again.', 'woocommerce-product-addons' ),
				'i18n_validation_error'                    => __( 'Failed to validate configuration. If this issue persists, please reload the page and try again.', 'woocommerce-product-addons' ),
	...
	public function convert_type_name( $type = '' ) {
		switch ( $type ) {
			case 'checkbox':
				$name = __( 'Checkboxes', 'woocommerce-product-addons' );
				break;
			case 'custom_price':
			...
			case 'datepicker':
				...
				break;
			case 'heading':
				$name = __( 'Heading', 'woocommerce-product-addons' );
				break;
			case 'multiple_choice':
			default:
