		...
		echo '<div class="booked-pending-appt-list booked-dashboard-widget">';
			...
			*/
			...
			$calendars = get_terms('booked_custom_calendars','orderby=slug&hide_empty=0');
                        ...
                        //if not an admin, show the ones this user can manage's assigned calendars, for example an agent user
                        if(!current_user_can('manage_booked_options') && !empty($calendars) && count($calendars)){
                         ...
                         //either they have a calendar assigned to them or they are admin
                        if($agent_query || current_user_can('manage_booked_options')){
                            //for those who are allowed to see
