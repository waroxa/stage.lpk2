...
if ( !isset($_GET['sh']) || isset($_GET['sh']) && $_GET['sh'] != BOOKEDICAL_SECURE_HASH )
	wp_die('<strong>Calendar Feed Requirements:</strong><br>The QuickCal calendar feeds now require a secure hash to access. Please take a look at your "Appointments > Calendar Feeds" page for the updated feed URLs.');


header('Content-type: text/calendar; charset=utf-8');
header('Content-Disposition: attachment; filename=appointment-feed-'.QUICKCAL_VERSION.'.ics');
	...
	foreach($appts_in_this_timeslot as $appt_id):
		...
		if (!$guest_name):
			...
			if ($user_id):
				...
				$display_name = quickcal_get_name($user_id);
				if ( !empty($user_info) ):
