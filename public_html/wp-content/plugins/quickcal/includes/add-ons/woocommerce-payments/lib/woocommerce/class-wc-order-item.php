...
class QuickCal_WC_Order_Item_Hooks {
	...
	public static function woocommerce_attribute_label($label, $name, $product=null) {
		...
		if ( preg_match('~cfield_\d+$~', strtolower($label)) ) {
			...
			add_filter('woocommerce_order_item_display_meta_value', array('QuickCal_WC_Order_Item_Hooks', 'woocommerce_order_item_display_custom_field_meta_value'), 10, 1);
		} else if ( QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_cal_name' === strtolower($label) ) {
			$label = __('Calendar', 'booked');
		} else if ( QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_assignee_name' === strtolower($label) ) {
			$label = __('Booking Agent', 'booked');
		}

		if ( $label && !strstr( QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_id', strtolower($label) ) ) {
			return $label;
		}

		add_filter('woocommerce_order_item_display_meta_value', array('QuickCal_WC_Order_Item_Hooks', 'woocommerce_order_item_display_meta_value'), 10, 1);

		if ( is_admin() ) {
			add_filter('pre_kses', array('QuickCal_WC_Order_Item_Hooks', 'pre_kses'), 10, 3);
		}
	...
	public static function woocommerce_order_item_display_meta_value( $meta_value ) {
		remove_filter('woocommerce_order_item_display_meta_value', array('QuickCal_WC_Order_Item_Hooks', 'woocommerce_order_item_display_meta_value'), 10, 1);

		try {
			$appointment = QuickCal_WC_Appointment::get(intval($meta_value));
		} catch (Exception $e) {
			$appointment = false;
		}
	...
	public static function woocommerce_order_item_display_custom_field_meta_value( $meta_value ) {
		remove_filter('woocommerce_order_item_display_meta_value', array('QuickCal_WC_Order_Item_Hooks', 'woocommerce_order_item_display_custom_field_meta_value'), 10, 1);
		$separator = '--SEP--';
	...
	public static function pre_kses($string, $allowed_html, $allowed_protocols) {
		...
		remove_filter('pre_kses', array('QuickCal_WC_Order_Item_Hooks', 'pre_kses'), 10, 3);
