...
class QuickCal_WC_Appointment_Payment_Status {
	public $app_id;
	public $order_id;
	...
	public function __construct($app_id) {
		if ( !is_integer($app_id) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'booked'), 'new QuickCal_WC_Appointment_Payment_Status::get($app_id)', gettype($app_id) );
			throw new Exception($message);
		} else if ( $app_id===0 ) {
			$message = sprintf( __('%s invalid ID is given. %s', 'booked'), 'new QuickCal_WC_Appointment_Payment_Status::get($app_id)', '$app_id=0' );
			throw new Exception($message);
		}
	...
	public function get_order() {
		$this->order_id = get_post_meta($this->app_id, '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_order_id', true);
		if ( $this->order_id && $this->order_id != 'manual') {
			$this->order_id = (int) $this->order_id;
			$this->order_obj = QuickCal_WC_Order::get($this->order_id);
		}
	...
	public function set_statuses() {
		...
		if ( $this->order_id && $this->order_id === 'manual' || $this->order_id && $this->order_obj->order->get_status() === 'wc-completed' || $this->order_id && $this->order_obj->order->get_status() === 'completed' ) {
			...
			$this->payment_status_text = __('Order ', 'booked') . $this->order_obj->get_status_text();
		}
