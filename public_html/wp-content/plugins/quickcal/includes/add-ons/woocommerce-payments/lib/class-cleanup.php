...
class QuickCal_WC_Cleanup {
	...
	protected function appointments_to_clean() {
		...
		$this->appointments = get_posts(array(
			'posts_per_page' => -1,
			'post_type' => QUICKCAL_WC_POST_TYPE,
			'post_status' => array('any'),
			'suppress_filters' => false,
			'_' . QUICKCAL_WC_PLUGIN_PREFIX . 'cleanup' => true
		));
	...
	public function pre_get_posts( $query ) {
		...
		if ( !$query->get( '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'cleanup') ) {
			return $query;
		}
	...
	public function posts_where( $where ) {
		...
		$time_created_meta_key = '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'time_created';
