...
class QuickCal_WC_Functions {
	...
	public static function get_login_page_info() {
		...
		$query = "SELECT * FROM `{$wpdb->posts}`
					...
					AND `post_content` LIKE '%[quickcal-login]%'";

		$pages = $wpdb->get_results($query);
		if ( !$pages ) {
                        //if not found, user might be using the older booked shortcodes.
	...
	public static function get_appointments_page_info() {
		...
		$query = "SELECT * FROM `{$wpdb->posts}`
					WHERE `post_type` = 'page'
					AND `post_content` LIKE '%[quickcal-appointments]%'";

		$pages = $wpdb->get_results($query);
		if ( !$pages ) {
                        //support old shortcode
                        $query2 = "SELECT * FROM `{$wpdb->posts}`
					WHERE `post_type` = 'page'
	...
	public static function booked_custom_field_data( $custom_field_data = array() ) {
		...
		foreach($custom_fields as $key => $field) {
			...
			$product = QuickCal_WC_Product::get($product_id);
		...
		if ( $has_product ) {
			...
			add_filter('booked_new_appointment_args', array('QuickCal_WC_Functions', 'booked_new_appointment_args_on_create'), 10, 1);
		}
	...
	public static function booked_prepare_sending_reminder( $send = true, $appt_id = false ) {
		...
		if ( is_array( $appt_id ) ):
			...
			if ( $is_wc_order ):
				$order = new QuickCal_WC_Appointment_Payment_Status( $appt_id );
	...
	// this hook should be called only in QuickCal_WC_Functions::booked_custom_field_data() only if product option is available
	public static function booked_new_appointment_args_on_create( $post_params=array() ) {

		// just in case, remove the filter
		remove_filter('booked_new_appointment_args', array('QuickCal_WC_Functions', 'booked_new_appointment_args_on_create'), 10, 1);

		// set post status to awaiting payment
		$post_params['post_status'] = QUICKCAL_WC_PLUGIN_PREFIX . 'awaiting';
	...
	public static function booked_appointments_array( $appointments_array ) {
        ...
        // remove appointments if their related orders doesn't exist
	public static function booked_appointments_array_cleanup( $appointments_array ) {
                foreach($appointments_array as $id=>$appt){
                    $order_id = get_post_meta($id, '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_order_id', true);
                    if($order_id){
                         $order_obj = QuickCal_WC_Order::get((int)$order_id);
                         if(!$order_obj->order){
                             error_log('order missing, remove it'. $id);
                             //invalid order, order missing, delete this appointment
                            wp_delete_post( $id, true );
                            unset($appointments_array[$id]);
                         }
                    }
                }
		return $appointments_array;
	...
	public static function booked_store_appointment_creation_date( $appointment_id=null ) {
		...
		update_post_meta($appointment_id, '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'time_created', $current_time);
		update_post_meta($appointment_id, '_' . QUICKCAL_WC_PLUGIN_PREFIX . 'date_created', date('Y-m-d H:i:s', $current_time));
	...
	public static function booked_new_appointment_created( $appointment_id = null ) {
		...
		$added_to_cart = QuickCal_WC_Cart::add_appointment($appointment_id);
		if ( !$added_to_cart ) {
			return;
		}
	...
	public static function booked_shortcode_appointments_buttons( $appointment_id=null ) {
		...
		$edit_button = QuickCal_WC_Fragments::get_path('appointment', 'buttons');
		include($edit_button);
	}

	public static function booked_shortcode_appointments_additional_information( $appointment_id=null ) {
		$additional_info = QuickCal_WC_Fragments::get_path('appointment', 'additional-info');
	...
	public static function booked_shortcode_appointments_allow_cancel($allow_cancel, $app_id) {
		$appointment = QuickCal_WC_Appointment::get(intval($app_id));
	...
	public static function booked_fea_shortcode_appointments_buttons($button_html, $appt_id) {
		$appointment = QuickCal_WC_Appointment::get(intval($appt_id));
	...
	public static function booked_before_creating_appointment() {
		...
		remove_action('booked_new_appointment_created', array('QuickCal_WC_Functions', 'booked_new_appointment_created'), 15, 1);

		// turn the wp_insert_post to act as wp_update_post
		add_filter('booked_new_appointment_args', array('QuickCal_WC_Functions', 'booked_new_appointment_args_on_date_change'), 10, 1);

		// dont allow to update the metas and the calendar term
		add_filter('booked_update_cf_meta_value', array('QuickCal_WC_Functions', 'return_false'), 10);
		add_filter('booked_update_appointment_calendar', array('QuickCal_WC_Functions', 'return_false'), 10);
	...
	public static function booked_disable_confirmation_emails(){
		if ( QuickCal_WC_Settings::get_option('email_confirmations') === 'after_complete' || QuickCal_WC_Settings::get_option('email_confirmations') === false ):
	...
	public static function remove_default_emails() {
		...
		if (isset($_POST)):
			...
			if ( $booked_wc_product && $booked_disable_confirmation_emails ):
				remove_action( 'booked_confirmation_email', 'quickcal_mailer', 10 );
				remove_action( 'booked_admin_confirmation_email', 'quickcal_mailer', 10 );
			endif;
	...
	public static function booked_new_appointment_args_on_date_change( $post_args ) {
		...
		$app_detailed_obj = QuickCal_WC_Appointment::get(intval($appointment_id));
		$awaiting_payment_status = QUICKCAL_WC_PLUGIN_PREFIX . 'awaiting';
	...
	public static function booked_admin_calendar_buttons_before($calendar_id, $appt_id, $status) {
		...
		add_filter('booked_admin_show_calendar_buttons', array('QuickCal_WC_Functions', 'return_false'), 10);

		$edit_button = QuickCal_WC_Fragments::get_path('admin-calendar', 'app-buttons');
		include($edit_button);
	}

	public static function booked_admin_calendar_buttons_after($calendar_id, $appt_id, $status) {
		remove_filter('booked_admin_show_calendar_buttons', array('QuickCal_WC_Functions', 'return_false'), 10);
	}
