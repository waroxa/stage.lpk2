...
class QuickCal_WC_Cart {
	...
	public static function add_appointment( $app_id=null ) {
		...
		foreach ($appointment->products as $product) {
			...
			$additional_item_data = array(
				QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_id' => $app_id
			);

			// add calendar name as part of the item data
			if ( !empty($appointment->calendar->calendar_obj) ) {
				$additional_item_data[QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_cal_name'] = $appointment->calendar->calendar_obj->name;
			}
			// <---
			...
			if ( !empty($appointment->calendar->calendar_obj) ) {
				...
				if ( $assignee_email && ($usr=get_user_by('email', $assignee_email)) ) {
					$additional_item_data[QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_assignee_name'] = $usr->display_name;
				}
			}
			// <---

			// add timerange name as part of the item data
			$additional_item_data[QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_timerange'] = $appointment->timeslot_text;
			// <---
	...
	public static function get_cart_appointments() {
		...
		if ( method_exists($cart, 'get_cart') ):
			...
			$app_id_key = QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_id';
...
class QuickCal_WC_Cart_Hooks {

    public static function woocommerce_checkout_fields( $fields ) {
        global $woocommerce;
        if ( isset($woocommerce->session) && !is_null($woocommerce->session) ) {
            ...
            error_log('WooCommerce session is not initialized.');
        }
	...
	public static function woocommerce_cart_item_thumbnail( $thumbnail, $cart_item ) {
		...
		$app_id_key = QUICKCAL_WC_PLUGIN_PREFIX . 'appointment_id';
		if ( isset($cart_item[$app_id_key]) ) {
			if ( QuickCal_WC_Settings::get_option('enable_thumbnails') === 'enable' ) {
				return $thumbnail;
			} else {
				return false;
			}
