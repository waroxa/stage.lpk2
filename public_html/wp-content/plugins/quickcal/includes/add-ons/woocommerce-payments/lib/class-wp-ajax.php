...
class QuickCal_WC_Ajax {
	...
	private function __construct() {
		...
		foreach ($functions as $ajax_type => $requests) {
			foreach ($requests as $action_value => $function_name) {
				$filter_name = $ajax_type . QUICKCAL_WC_PLUGIN_PREFIX . $action_value;
	...
	public function load_product_variations() {
		...
		try {
			...
			$fragment_file = QuickCal_WC_Fragments::get_path('ajax-loaded/product','variations');
	...
	public function add_appointment_to_cart() {
		$response = new QuickCal_WC_Response();
		...
		try {
			...
			QuickCal_WC_Cart::add_appointment($app_id);
		} catch (Exception $e) {
			$response->add_message($e->getMessage());
			$response->create();
		}
