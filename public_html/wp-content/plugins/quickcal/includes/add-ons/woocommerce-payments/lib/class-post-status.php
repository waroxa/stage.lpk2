...
class QuickCal_WC_Post_Status {
	...
	protected function register_status() {
		register_post_status(
			QUICKCAL_WC_PLUGIN_PREFIX . 'awaiting',
			array(
	...
	public function append_post_status_list() {
		...
		if( $post && $post->post_type===QUICKCAL_WC_POST_TYPE ) {
			$post_status = QUICKCAL_WC_PLUGIN_PREFIX . 'awaiting';
			$complete = '';
			$label = '';
