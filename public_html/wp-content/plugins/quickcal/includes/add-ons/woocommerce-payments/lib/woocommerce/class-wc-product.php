...
class QuickCal_WC_Product {
	...
	public static function get( $post_id = null ) {
		...
		if ( !is_integer( $post_id ) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'booked'), 'QuickCal_WC_Product::get($post_id)', gettype($post_id) );
	...
	protected function get_data() {
		$product = $this->_get_product_data();
                if(!$product){
                    return false;
                }
	...
	protected function _get_variations() {
		if ( $this->type==='variable' ) {
			...
			add_filter('woocommerce_available_variation', array('QuickCal_WC_Variation', 'woocommerce_available_variation'), 10, 3);
			$product_variations = $this->data->get_available_variations();
			remove_filter('woocommerce_available_variation', array('QuickCal_WC_Variation', 'woocommerce_available_variation'));
