						...
						echo '<span class="appt-block bookedClearFix '.(!$historic ? $status_class : 'approved').'" data-appt-id="'.$appt['post_id'].'">';
							...
							if (!$historic):
                                ...
                                quickcal_add_to_calendar_button($calendar_button_array,$cf_meta_value);
                                                                ?>
                                                                <?php
								if ($appt_date_time >= $date_to_compare && apply_filters('booked_shortcode_appointments_allow_change', true, $appt['post_id']) && !get_option('booked_dont_allow_user_changes',false) ) {
                                                                    //check if all the plugins are okay to show the button or have specific checks
                                                                    $appt['calendar_info'] = quickcal_set_appointment_calendar($appt['post_id']); 

                                                                    if(!empty($appt['calendar_info']->calendar_link)){
                                                                        $calendar_url = $appt['calendar_info']->calendar_link;
                                                                        ...
                                                                        ?>
                                                                        <a href="<?php echo $calendar_link ?>" data-app-calendar="<?php echo $calendar_link ?>" data-appt-id="<?php echo $appt['post_id'] ?>" class="edit"><?php _e('Change Date', 'booked'); ?></a>
                                                                        <?php 
                                                                    }
								...
								if ( apply_filters('booked_shortcode_appointments_allow_cancel', true, $appt['post_id']) && !get_option('booked_dont_allow_user_cancellations',false) ) {
                                                                     //check if all the plugins are okay to show the button or have specific checks
                                                                     if ( $appt_date_time >= $date_to_compare && apply_filters('booked_check_cancel_button_conditions', true, $appt['post_id'])) {
