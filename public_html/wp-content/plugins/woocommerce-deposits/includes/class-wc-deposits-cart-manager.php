...
class WC_Deposits_Cart_Manager {
	...
	public function __construct() {
		...
		add_filter( 'wcpay_payment_request_hide_itemization', array( $this, 'wcpay_payment_request_hide_itemization' ) );
	}
	 ...
	 *
	 * The full amount is calculated, displayed and charged correctly, ie it accounts
	 * for the deposit, but the itemized amount is incorrect leading to confusion as
	...
	public function wp_enqueue_scripts() {
		wp_enqueue_style( 'wc-deposits-frontend', WC_DEPOSITS_PLUGIN_URL . '/build/frontend.css', null, WC_DEPOSITS_VERSION );
		WC_Deposits::register_script( 'wc-deposits-frontend', 'frontend' );
	}
	...
	public function is_deposit_order() {
		...
		if ( ! empty( $order_id ) ) {
			...
			if (
				$order &&
				(
					WC_Deposits_Order_Manager::is_follow_up_order( $order ) ||
					$order->has_status( 'pending-deposit' ) ||
					$order->has_status( 'scheduled-payment' )
				)
			) {
				return true;
			}
	...
	public static function get_global_param( $name ) {
		...
		// the value might not be always set.
		$php_input = file_get_contents( 'php://input' );
		if ( false !== $php_input && strlen( $php_input ) > 0 ) {
			$decoded_input = json_decode( $php_input, true );

			if ( isset( $decoded_input[ $name ] ) && ! empty( $decoded_input[ $name ] ) ) {
				return sanitize_text_field( wp_unslash( $decoded_input[ $name ] ) );
			}
		}
		// phpcs:enable
	...
	public function validate_add_cart_item( $passed, $product_id, $qty, $variation_id = 0, $variations = array(), $cart_item_data = array() ) {
		 ...
		 */
		if ( $product->is_sold_individually() && 'booking' !== $product->get_type() && 'accommodation-booking' !== $product->get_type() ) {
			foreach ( WC()->cart->get_cart() as $cart_item ) {
				if ( $cart_item['product_id'] === $product_id ) {
					/* translators: %s: product name */
					$message         = sprintf( __( 'You cannot add another "%s" to your cart.', 'woocommerce-deposits' ), $product->get_name() );
					$wp_button_class = wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '';
					wc_add_notice(
						sprintf( '%s <a href="%s" class="button wc-forward%s">%s</a>', esc_html( $message ), esc_url( wc_get_cart_url() ), esc_attr( $wp_button_class ), esc_html__( 'View cart', 'woocommerce-deposits' ) ),
						'error'
					);
					return false;
				}
	...
	private function cart_totals_tax_string() {
		...
		if ( ! empty( $tax_string_array ) ) {
			...
			$estimated_text = WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ? sprintf( ' ' . __( 'estimated for %s', 'woocommerce-deposits' ), WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] ) : '';
			/* translators: %s: tax information */
			$value .= '<small class="includes_tax">' . sprintf( __( '(includes %s)', 'woocommerce-deposits' ), implode( ', ', $tax_string_array ) . $estimated_text ) . '</small>';
		}
		return $value;
	}
