...
class WC_Deposits_Store_API {
	...
	public static function filter_cart_item_data( $response, $server, $request ) {
		...
		foreach ( $data['items'] as $key => $item_data ) {
			...
			/**
			 * Filter hook to modify the deposit amount.
			 *
			 * @param float      $cart_item_deposit_amount Deposit amount.
			 * @param WC_Product $cart_item_product Cart item data.
			 *
			 * @since 2.2.9
			 *
			 * @return float
			 */
			$cart_item_deposit_amount     = apply_filters( 'woocommerce_deposits_get_deposit_amount', $cart_item['deposit_amount'], $cart_item['data'] ); // We need to apply this filter to the deposit amount, as it may have been affected by Memberships.
			...
			if ( 'excl' === WC()->cart->get_tax_price_display_mode() ) {
				...
				$adjusted_values = self::maybe_adjust_order_line_item_prices( $deposit_amount_including_tax, $deposit_amount_excluding_tax );

				$data['items'][ $key ]['totals']->line_subtotal     = $adjusted_values['line_subtotal'];
				$data['items'][ $key ]['totals']->line_subtotal_tax = $adjusted_values['line_subtotal_tax'];
			}
		}
	...
	/**
	 * Adjusts the order-line item prices for the frontend.
	 *
	 * @param float $deposit_amount_including_tax Deposit amount including tax.
	 * @param float $deposit_amount_excluding_tax Deposit amount excluding tax.
	 *
	 * @see https://github.com/woocommerce/woocommerce/discussions/46126
	 *
	 * @return array
	 */
	public static function maybe_adjust_order_line_item_prices( $deposit_amount_including_tax, $deposit_amount_excluding_tax ) {
		$decimals                     = wc_get_price_decimals();
		$deposit_amount_excluding_tax = round( $deposit_amount_excluding_tax, $decimals );
		$deposit_amount_excluding_tax = round( $deposit_amount_excluding_tax, $decimals );

		$delta = $deposit_amount_including_tax - $deposit_amount_excluding_tax;

		if ( 0 === $delta ) {
			return array(
				'line_subtotal'     => $deposit_amount_including_tax,
				'line_subtotal_tax' => 0,
			);
		}

		if ( $decimals > 0 ) {
			$adjusted_price = ( $deposit_amount_excluding_tax ) * 10 ** $decimals;
			$adjusted_tax   = ( $deposit_amount_including_tax - $deposit_amount_excluding_tax ) * 10 ** $decimals;
		} else {
			$deposit_amount_excluding_tax_integer_part = (int) $deposit_amount_excluding_tax;
			$deposit_amount_tax_integer_part           = (int) ( $deposit_amount_including_tax - $deposit_amount_excluding_tax );

			$delta          = $deposit_amount_including_tax - ( $deposit_amount_excluding_tax_integer_part + $deposit_amount_tax_integer_part );
			$adjusted_price = $deposit_amount_excluding_tax_integer_part + ( $delta >= 0.5 ? ceil( $delta ) : floor( $delta ) );
			$adjusted_tax   = $deposit_amount_tax_integer_part;
		}

		return array(
			'line_subtotal'     => $adjusted_price,
			'line_subtotal_tax' => $adjusted_tax,
		);
	}
