		...
		if (!empty($args['more_text']) || (int)$meta['product'] > 0) {
			?><div class="sc_dishes_item_button sc_item_button"><?php
				...
				if ((int)$meta['product'] > 0) {
					?><a href="<?php the_permalink($meta['product']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_simple sc_dishes_button_order', 'sc_dishes', $args)); ?>"><?php esc_html_e('Order now', 'trx_addons'); ?></a><?php
				}
				?>
			</div><?php
		}
