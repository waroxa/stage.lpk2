...
if (have_posts()) {
	...
	?><div class="sc_team sc_team_default">
		...
		<div class="sc_team_columns_wrap <?php echo esc_attr(trx_addons_get_columns_wrap_class()); ?> columns_padding_bottom"><?php
			...
			$trx_addons_team_type = empty($trx_addons_team_style[0]) ? 'default' : $trx_addons_team_style[0];
			$trx_addons_team_columns = empty($trx_addons_team_style[1]) ? 2 : max(1, $trx_addons_team_style[1]);
