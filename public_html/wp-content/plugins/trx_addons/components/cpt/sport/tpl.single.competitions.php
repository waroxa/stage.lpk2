					...
					foreach ($rounds as $id=>$name) {
						...
						foreach ($matches as $id_match=>$name_match) {
							...
							if ($last_date != $meta['date_start']) {
								...
								// Round name and date
								?><li><a href="<?php the_permalink($id); ?>" class="sport_page_list_link"><?php
							...
							?><li class="sport_page_list_round_<?php echo esc_html($id.'_'.$last_date); ?>"><a href="<?php the_permalink($id_match); ?>" class="sport_page_list_link"><?php
								// Start time
