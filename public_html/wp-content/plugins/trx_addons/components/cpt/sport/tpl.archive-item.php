		...
		if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) {
			...
			if (false) {
				...
				<div class="sc_item_button sc_sport_item_button<?php if (!empty($args['slug'])) echo ' sc_'.esc_attr($args['slug']).'_item_button'; ?>"><?php
					// More
					if (false) {
						?><a href="<?php the_permalink(); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_simple', 'sc_sport', $args)); ?>"><?php esc_html_e('More', 'trx_addons'); ?></a><?php
					}
					if ($args['slug']=='competitions') {
						// Rounds
