...
if (!empty($meta['player1']) && !empty($meta['player2'])) {
	...
	?><div data-post-id="<?php echo esc_attr($match['id']); ?>" class="sc_matches_item sc_matches_item_pair"><?php
		...
		?><div class="sc_matches_item_player sc_matches_item_player1"><?php
			...
			// Name or Command
			?><h5 class="sc_matches_item_player_name"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h5><?php
			// Club or Country
		...
		?><div class="sc_matches_item_score"><?php
			if (!$is_match_page) {
				?><a href="<?php the_permalink($match['id']); ?>" class="sc_matches_item_link"><?php
			}
			...
			if (!empty($meta['review']) && !trx_addons_is_off($meta['review'])) {
				?><a class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_matches_item_review_link theme_button', 'sc_matches', $args)); ?>" href="<?php the_permalink($meta['review']); ?>"><span class="sc_matches_item_review_title"><?php esc_html_e('Match Review', 'trx_addons'); ?></span></a><?php
			}
			if (!$is_match_page) {
				?></a><?php
			}
