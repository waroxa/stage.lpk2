...
if (have_posts()) {
	...
	$trx_addons_services_type = empty($trx_addons_services_style[0]) ? 'default' : $trx_addons_services_style[0];
	$trx_addons_services_columns = empty($trx_addons_services_style[1]) ? 2 : max(1, $trx_addons_services_style[1]);
		...
		<div class="sc_services_columns_wrap sc_item_columns columns_padding_bottom sc_item_columns_<?php
			...
			while ( have_posts() ) { the_post(); 
				...
				trx_addons_get_template_part(array(
												TRX_ADDONS_PLUGIN_CPT . 'services/tpl.'.trim( trx_addons_sanitize_file_name( $trx_addons_services_type ) ).'-item.php',
