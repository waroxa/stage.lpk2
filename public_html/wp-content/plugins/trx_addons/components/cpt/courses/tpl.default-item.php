...
<div data-post-id="<?php the_ID(); ?>" class="sc_courses_item trx_addons_hover trx_addons_hover_style_links">
		...
		?><div class="sc_courses_item_image"><?php
			trx_addons_get_template_part('templates/tpl.featured.php',
										...
										apply_filters('trx_addons_filter_args_featured', array(
														...
														'hover' => 'zoomin',
														'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size((int)$args['columns'] > 2 ? 'medium' : 'big'), 'courses-default')
														),
														'courses-default'
													)
										);
	...
	?><div class="sc_courses_item_info">
		<div class="sc_courses_item_header">
			<h4 class="sc_courses_item_title entry-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h4>
			<div class="sc_courses_item_meta">
				<a href="<?php the_permalink(); ?>">
					<span class="sc_courses_item_meta_item sc_courses_item_meta_date"><?php
						$dt = $meta['date'];
						echo sprintf($dt < date_i18n('Y-m-d') ? esc_html__('Started on %s', 'trx_addons') : esc_html__('Starting %s', 'trx_addons'), '<span class="sc_courses_item_date">' . date_i18n(get_option('date_format'), strtotime($dt)) . '</span>');
					?></span>
		...
		if (!empty($meta['product']) && (int) $meta['product'] > 0) {
			?><a href="<?php the_permalink($meta['product']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_simple sc_courses_item_button', 'sc_courses', $args)); ?>"><?php esc_html_e('Buy Now', 'trx_addons'); ?></a><?php
		}
	?></div><?php
		...
		<div class="trx_addons_hover_content">
			...
			if (!empty($args['more_text']) || (!empty($meta['product']) && (int) $meta['product'] > 0)) {
				?><div class="trx_addons_hover_links"><?php
					...
					if (!empty($meta['product']) && (int) $meta['product'] > 0) {
						?><a href="<?php the_permalink($meta['product']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'trx_addons_hover_link2', 'sc_courses', $args)); ?>"><?php esc_html_e('Buy Now', 'trx_addons'); ?></a><?php
					}
				?></div><?php
			}
		?></div><?php
