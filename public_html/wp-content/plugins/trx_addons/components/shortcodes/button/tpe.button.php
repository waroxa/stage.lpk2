...
#><div id="{{ id }}" class="sc_item_button sc_button_wrap sc_align_{{ settings.align }}"><#
	...
	_.each(settings.buttons, function(item) {
		...
		#>><#
			...
			// Icon or Image
			if (!trx_addons_is_off(item.image.url) || !trx_addons_is_off(item.icon)) {
				#><span class="sc_button_icon"><#
					if (item.image.url != '') {
						...
						if (item.icon.indexOf('.svg') >= 0) {
							#><object type="image/svg+xml" data="{{ item.icon }}" border="0"></object><#
						} else {
							#><img class="sc_icon_as_image" src="{{ item.icon }}" alt="<?php esc_attr_e('Icon', 'trx_addons'); ?>"><#
						}
					} else {
