...
if ( (int)$query->found_posts > 0 || count($tabs) > 0 ) {
	...
	?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?>
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			if ( (int)$posts_in_grid > 0 ) {
				if (($args['item_number'] - 1) % (int)$posts_in_grid == 0 ) {
			...
			if ( !apply_filters('trx_addons_filter_sc_blogger_template', false, $args) ) {
				$args['item_number'] += !trx_addons_is_off($args['pagination']) && (int)$args['page'] > 1 ? ( (int)$args['page'] - 1 ) * $args['count'] : 0;
